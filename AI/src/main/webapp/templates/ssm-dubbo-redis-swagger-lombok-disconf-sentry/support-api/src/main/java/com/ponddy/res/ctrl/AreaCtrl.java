...
import ${basePackage}.area.entity.CountryCitys;
import ${basePackage}.area.service.BasicAreaSV;
import ${basePackage}.area.service.CountryCitysSV;
import ${basePackage}.area.service.CountrySV;
import ${basePackage}.area.vo.CountryCitysQuery;
import ${basePackage}.area.vo.CountryQuery;
...
public class AreaCtrl extends BaseCtrl {
    ...
    @Reference
    private CountryCitysSV countryCitysSV;


    /**
     * 获取国家代码
     */
    @ApiOperation(value = "获取国家代码", notes = "获取国家代码")
    @GetMapping("/countries")
    @ResponseBody
    public BeanRet countries() {
        List<Country> countries = countrySV.getAll();
        logger.info("获取国家代码 : " + JSON.toJSONString(countries));
        return BeanRet.create(true, "", StringTools.convertList(countries, CountryQuery.class));
    }

    /**
     * 获取国家下的城市
     */
    @ApiOperation(value = "获取国家下的城市", notes = "获取国家下的城市")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "countryCode", value = "省区域编码", required = true, paramType = "path")
    })
    @GetMapping("/cities")
    @ResponseBody
    public BeanRet cities(String countryCode) {
        Assert.hasText(countryCode, BaseException.BaseExceptionEnum.Empty_Param.toString());
        Map<String, Object> map = Maps.newHashMap();
        map.put("countryCode", countryCode);
        List<CountryCitys> countries = countryCitysSV.queryList(map);
        logger.info("获取国家下的城市 : " + JSON.toJSONString(countries));
        return BeanRet.create(true, "", StringTools.convertList(countries, CountryCitysQuery.class));
    }
