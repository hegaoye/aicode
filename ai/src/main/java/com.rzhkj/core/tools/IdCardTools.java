...
import com.ponddy.core.enums.SexEnum;
...
public class IdCardTools {
    ...
    public boolean checkIdCard(String idCard) {
        //调用工具类校验身份证号格式是否正确
        if (!StringTools.checkIdCard(idCard)) return false;
        ...
        if (map == null) {
            ...
            if (!StringTools.isNotEmpty(area_cache)) return false;
            map = JSON.parseObject(area_cache);
        }
        if (map == null) return false;

        //检查身份证号区域是否存在
        if (map.containsKey(areaCode)) return true;
        return false;
    }
