package com.rzhkj.core.tools.memcached;
...
public class ScanFileToCached implements IScanFileToCached {
    ...
    public List<String> listFilesPath(File path) throws Exception {
        ...
        for (int i = 0; i < fs.length; i++) {
            if (fs[i].isDirectory()) {
                ...
                if (abpath.contains(".css")) css++;
                if (abpath.contains(".js")) js++;
                if (abpath.contains(".html")) html++;
                if (abpath.contains(".jpg")) jpg++;
                if (abpath.contains(".png")) png++;
                if (abpath.contains(".ico")) ico++;
                if (abpath.contains(".gif")) gif++;
    ...
    public List<File> listFiles(List<String> paths) throws FileNotFoundException {
        if (paths == null || paths.isEmpty()) throw new FileNotFoundException("扫描路径集合为空异常！");
        ...
        if (fileList.isEmpty()) throw new FileNotFoundException("没有扫描到文件！");
        return fileList;
    }

    @Override
    public File getFile(String path) throws FileNotFoundException {
        if (StringUtils.isBlank(path)) throw new FileNotFoundException("路径为空！");
    ...
    public boolean cacheFile(String key, File file) throws Exception {
        if (file == null) throw new FileNotFoundException("文件不存在！无法缓存！");
